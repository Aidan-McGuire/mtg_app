...
- **Known data-completeness gap (pre-existing importer limitation):** Scryfall's bulk data omits the top-level `colors` field for double-faced and transform cards; colors for those cards live only in `card_faces[*].colors`. This project's importer (`importer.py`) only reads the top-level field, so it stores `colors = ''` (empty) for these cards even though they are colored — e.g., "Bala Ged Recovery // Bala Ged Sanctuary" (identity G), "Brutal Cathar // Moonrage Brute" (identity RW), "Agadeem's Awakening // Agadeem, the Undercrypt" (identity B). This affects roughly 895 colored non-land cards, approximately 419 of them double-faced specifically. As a result, the "Exact Colors" filter gives incorrect results on these cards (a green DFC won't show up under Exact Colors = G; selecting Exact Colorless will incorrectly include them). Fix requires updating `importer.py` to fall back to the union of `card_faces[*].colors` when the top-level field is absent, plus a full re-import of the ~36k-card database. This is not a bug in this feature's filter logic (the SQL/JS comparison is correct), but a pre-existing gap in the underlying `colors` data.
